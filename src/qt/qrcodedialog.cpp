...
{
    QString ret = QString("hyper:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non BTC unit input in lnReqAmount, we generate the URI with HYPER as unit (as defined in BIP21)
