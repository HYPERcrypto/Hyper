...
bool AppInit(int argc, char* argv[])
{
    ...
    try
    {
        ...
        if (mapArgs.count("-?") || mapArgs.count("--help"))
        {
            ...
            std::string strUsage = _("Hyper version") + " " + FormatFullVersion() + "\n\n" +
                _("Usage:") + "\n" +
                  "  Hyperd [options]                     " + "\n" +
                  "  Hyperd [options] <command> [params]  " + _("Send command to -server or Hyperd") + "\n" +
                  "  Hyperd [options] help                " + _("List commands") + "\n" +
                  "  Hyperd [options] help <command>      " + _("Get help for a command") + "\n";
        ...
        for (int i = 1; i < argc; i++)
            if (!IsSwitchChar(argv[i][0]) && !boost::algorithm::istarts_with(argv[i], "Hyper:"))
                fCommandLine = true;
...
std::string HelpMessage()
{
    string strUsage = _("Options:") + "\n" +
        "  -?                     " + _("This help message") + "\n" +
        "  -conf=<file>           " + _("Specify configuration file (default: Hyper.conf)") + "\n" +
        "  -pid=<file>            " + _("Specify pid file (default: Hyperd.pid)") + "\n" +
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s.  Hyper is probably already running."), strDataDir.c_str()));
    ...
    if (fDaemon)
        fprintf(stdout, "Hyper server starting\n");
    ...
    if (nLoadWalletRet != DB_LOAD_OK)
    {
        ...
        {
            strErrors << _("Wallet needed to be rewritten: restart Hyper to complete") << "\n";
